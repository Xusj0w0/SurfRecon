...
class MeshRendererMixin:
    def render_mesh(
        ...
        **kwargs,
        ...
        assert getattr(pc, "mesh", None) is not None
        full_mesh: Meshes = pc.mesh
        # filter by camera frustum
        vert_mask = self.is_in_view_frustum(full_mesh.verts, viewpoint_camera)
        face_mask = vert_mask[full_mesh.faces].any(dim=-1)
        # mesh = full_mesh.submesh(face_mask=face_mask)
        mesh = Meshes(verts=full_mesh.verts, faces=full_mesh.faces[face_mask])

        # Rasterize
        if max_triangles_in_batch < 0:
            rast_out, verts_in_ndc = self.nvdiff_rasterization(viewpoint_camera, mesh.verts, mesh.faces, self._gl_context)
        else:
            idx_shift = 0
            rast_out: RasterizerOutputs = None
            for st in range(0, mesh.faces.shape[0], max_triangles_in_batch):
                ed = min(mesh.faces.shape[0], st + max_triangles_in_batch)
                sub_rast_out, verts_in_ndc = self.nvdiff_rasterization(viewpoint_camera, mesh.verts, mesh.faces[st:ed], self._gl_context)
                if rast_out is None:
        ...
        color_required = "mesh_rgb" in render_types and mesh.verts_colors is not None
        ...
        if depth_required:
            ...
            verts_h = torch.cat([mesh.verts, mesh.verts.new_ones((len(mesh.verts), 1))], dim=1)
        ...
        if color_required:
            ...
            features = torch.cat([features, mesh.verts_colors], dim=-1)

        # Compute image
        feature_img, _ = dr.interpolate(features[None], rast_out.rast_out, mesh.faces)  # (1, H, W, n_features)

        # Antialiasing for propagating gradients
        if self.config.mesh_rast_config.anti_aliased:
            feature_img = dr.antialias(feature_img, rast_out.rast_out, verts_in_ndc, mesh.faces)

        output_pkg = {}
        output_pkg["mesh_rgb"] = feature_img[..., color_idx : color_idx + 3][0] if color_required else None
        output_pkg["mesh_depth"] = feature_img[..., depth_idx : depth_idx + 1][0] if depth_required else None
        ...
        if normal_required:
            ...
            if self.config.mesh_rast_config.check_errors:
                error_mask = pix_to_face >= mesh.faces.shape[0]
                error_encourtered = torch.sum(error_mask)
                if error_encourtered > 0:
                    pix_to_face = torch.clamp(pix_to_face, min=0, max=mesh.faces.shape[0] - 1)
                    valid_mask = valid_mask & ~error_mask
            output_pkg["mesh_normal"] = (mesh.face_normals[pix_to_face].squeeze()[None] * valid_mask)[0]
    ...
    def is_in_view_frustum(points: torch.Tensor, camera: Camera):
        pts_ndc = torch.cat([points, points.new_ones((len(points), 1))], dim=1) @ camera.full_projection
        pts_ndc = pts_ndc[..., :2] / (pts_ndc[..., -1:] + 1e-8)
        valid_mask = (pts_ndc > -1.1).all(dim=-1) & (pts_ndc < 1.1).all(dim=-1)
        return valid_mask
