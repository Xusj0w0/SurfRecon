...
class _RasterizeGaussians(torch.autograd.Function):
    ...
    def forward(
        ...
        if raster_settings.debug:
            ...
            try:
                num_rendered, color, coord, mcoord, alpha, normal, depth, mdepth, radii, num_hit_pixels, geomBuffer, binningBuffer, imgBuffer = _C.rasterize_gaussians(*args)
        ...
        ctx.save_for_backward(colors_precomp, means3D, scales, rotations, cov3Ds_precomp, normal, radii, num_hit_pixels, sh, geomBuffer, binningBuffer, imgBuffer, alpha)
        return color, radii, num_hit_pixels, coord, mcoord, depth, mdepth, alpha, normal

    @staticmethod
    def backward(ctx, grad_color, grad_radii, grad_num_hit_pixels, grad_coord, grad_mcoord, grad_depth, grad_mdepth, grad_alpha, grad_normal):

        # Restore necessary values from context
        num_rendered = ctx.num_rendered
        raster_settings = ctx.raster_settings
        colors_precomp, means3D, scales, rotations, cov3Ds_precomp, normal, radii, num_hit_pixels, sh, geomBuffer, binningBuffer, imgBuffer, alpha = ctx.saved_tensors
