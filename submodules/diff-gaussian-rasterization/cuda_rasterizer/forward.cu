...
__global__ void __launch_bounds__(BLOCK_X *BLOCK_Y) rasterizeImportanceCUDA(
  ...
  if (updated) {
    atomicAdd(&(num_max_pixels[idx_max]), 1);
  }
...
__global__ void preprocessWithZBufCUDA(
  ...
  float2 point_image = {ndc2Pix(p_proj.x, W), ndc2Pix(p_proj.y, H)};
  if (zbuf != nullptr) {
    float _z = __ldg(&zbuf[int(point_image.y) * W + int(point_image.x)]);
    if (p_view.z > _z + tolerance)
      return;
  }
...
void FORWARD::preprocess_with_zbuf(
    ...
    const dim3 grid, uint32_t *tiles_touched, const float *zbuf,
    const float tolerance, bool prefiltered) {
  preprocessWithZBufCUDA<NUM_CHANNELS><<<(P + 255) / 256, 256>>>(
      P, D, M, means3D, scales, scale_modifier, rotations, opacities, shs,
      clamped, cov3D_precomp, colors_precomp, viewmatrix, projmatrix, cam_pos,
      W, H, tan_fovx, tan_fovy, focal_x, focal_y, radii, means2D, depths,
      cov3Ds, rgb, conic_opacity, grid, tiles_touched, zbuf, tolerance,
      prefiltered);
}